...
from models import align_dlib as ad

logger = logging.getLogger(__name__)

align_dlib = ad.AlignDlib(os.path.join(os.path.dirname(__file__), 'shape_predictor_68_face_landmarks.dat'))
...
def main(input_dir, output_dir, crop_dim):
    ...
    for index, image_path in enumerate(image_paths):
        ...
        # raise ValueError(image_path, output_path, crop_dim)
...
def _align_image(image, crop_dim):
    ...
    aligned = align_dlib.align(crop_dim, image, bb, landmarkIndices=ad.AlignDlib.INNER_EYES_AND_BOTTOM_LIP)
